...
public class WMG_Editor : MonoBehaviour {
	...
	void OnNodePress(GameObject go, bool pressed) {
		...
		if (UICamera.currentTouchID == -2) {
			if (pressed) {
				...
				fromNode.numLinks++;
				dumLink.Setup(fromNode.gameObject, dummyNode, -1);
	...
	void OnEditorBackgroundDropped(GameObject go, GameObject orig) {
	...
	void OnSelectionChangePrefabNodes() {
		UIPopupList preNodesList = prefabNodesList.GetComponent<UIPopupList>();
	...
	void OnSelectionChangePrefabLinks() {
		UIPopupList preLinksList = prefabLinksList.GetComponent<UIPopupList>();
	...
	void OnSelectionChangeGraphTypes() {
